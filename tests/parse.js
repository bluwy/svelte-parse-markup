...
test('ast start and end is consistent', async () => {
  ...
  assert.equal(
    ...
    them.instance.content.body[0].start
  )
  assert.equal(
    mine.instance.content.body[mine.instance.content.body.length - 1].end,
    them.instance.content.body[them.instance.content.body.length - 1].end
  )
  assert.equal(
  ...
  assert.equal(
    ...
    them.module.content.body[0].start
  )
  assert.equal(
    mine.module.content.body[mine.module.content.body.length - 1].end,
    them.module.content.body[them.module.content.body.length - 1].end
  )
  assert.equal(
  ...
  assert.equal(mine.css.children[0].start, them.css.children[0].start)
  assert.equal(
    mine.css.children[mine.css.children.length - 1].end,
    them.css.children[them.css.children.length - 1].end
  )
  assert.equal(
